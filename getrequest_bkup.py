...
for i in orderbook.json()["result"]["buy"]:
    ...
    if (v_buybtc>1):
        if(n<v_width):
            print (colored(format(v_buybtc,'0.8f'),"blue") + "@" + colored(format(v_buyrate,'0.8f'),"red"))
            n +=1
cprint(("Total Buy: " + format(v_buytot,'0.2f') + "BTC"),'white', 'on_red')
...
for h in orderbook.json()["result"]["sell"]:
    ...
    if (v_sellbtc>1):
        if(j<v_width):
            print (colored(format(v_sellbtc,'0.8f'),"green") + "@" + colored(format(v_sellrate,'0.8f'),"red"))
            j +=1
cprint(("Total Sell: " + format(v_selltot,'0.2f') + "BTC"),'white', 'on_green')
